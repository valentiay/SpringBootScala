...
import com.ironworkman.db.{WorkPeriodRepository, WorkPeriodsDaysAndTimes, WorkPeriodsDaysAndTimesRepository}
...
class IronWorkManBot(val userRepository: UserRepository,
  ...
  def elapsed(count: Long = 0, duration: Long, amount: Long,
              chatId: Long, input: String): IO[Unit] =
    for {
      ...
      _ <- inputs match {
        ...
        case _ => for {
          ...
          _ <- sendMessageIO(chatId, s"Print for $count  1,2,3 categories separated by ;")
          _ <- Timer[IO].sleep(duration second)
        } yield ()
      }
    } yield ()
